...
class EventManager {
    private events: HarmonyEvent[] = [];
    private readonly storageKey = 'harmonyHubEvents';
    ...
    private handleEventSubmission(e: SubmitEvent): void {
        e.preventDefault();
    ...
    private addEvent(event: HarmonyEvent): void {
    ...
    private displayEvents(): void {
        ...
        const template = document.getElementById('eventTemplate') as HTMLTemplateElement;
        if (!template) {
        ...
        sortedEvents.forEach(event => {
            ...
            const eventElement = clone.querySelector('.event-item') as HTMLElement;
            ...
            if (eventElement) {
                ...
                if (dateElement) dateElement.textContent = new Date(event.date).toLocaleDateString();
...
export function initializeEventsPage(): void {
